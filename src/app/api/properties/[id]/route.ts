...
export async function PUT(
  ...
  try {
    ...
    if (admin.role !== "super_admin" && !admin.permissions?.manageProperties) {
      return NextResponse.json(
        { success: false, message: "Forbidden. You do not have permission to update listings." },
        { status: 403 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (admin.role !== "super_admin" && !admin.permissions?.manageProperties) {
      return NextResponse.json(
        { success: false, message: "Forbidden. You do not have permission to delete listings." },
        { status: 403 }
      );
    }
