      ...
      <div className="relative z-10 w-full max-w-md bg-white border border-zinc-200 p-8 rounded-3xl shadow-xl space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-zinc-500">
            Sign up to start your tenancy application
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 disabled:from-zinc-200 disabled:to-zinc-300 disabled:text-zinc-400 text-white font-bold py-3.5 rounded-2xl text-xs transition-all shadow-lg shadow-amber-500/10 active:scale-[0.98] cursor-pointer flex items-center justify-center gap-1.5 mt-2"
          >
            {loading ? (
              <>
        ...
        <div className="text-center text-xs text-zinc-500">
          ...
          <Link href={`/login?redirect=${encodeURIComponent(redirect)}`} className="text-amber-600 font-semibold hover:underline">
            Sign In
          </Link>
        </div>
      </div>
  ...
  return (
    <Suspense fallback={
      <div className="flex min-h-screen items-center justify-center bg-zinc-50">
        <div className="flex flex-col items-center gap-3">
          <div className="h-10 w-10 animate-spin rounded-full border-4 border-amber-500 border-t-transparent" />
          <p className="text-xs text-zinc-400 font-semibold tracking-wider uppercase">Loading Signup...</p>
        </div>
      </div>
    }>
      <SignupContent />
