...
export default function AdminDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"applications" | "properties" | "inquiries">("applications");
  ...
  useEffect(() => {
    async function checkAndLoad() {
      try {
        ...
        await Promise.all([fetchProperties(), fetchApplications(), fetchInquiries()]);
        setLoading(false);
      } catch (err) {
  ...
  const handleMutateStatus = async (appId: string, newStatus: "Approved" | "Rejected") => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // Update in-state application
        setApplications((prev) =>
          prev.map((app) => (app._id === appId ? { ...app, status: newStatus } : app))
        );
      } else {
        alert(data.message || "Failed to mutate status");
      }
    } catch (err) {
    ...
    try {
      const res = await fetch(`/api/properties/${propId}`, {
        ...
        body: JSON.stringify({
          ...
          availableDays: !currentAvailable ? 0 : 30 // defaults 30 days countdown if toggled rented
        })
      });
      const data = await res.json();
                    ...
                    {applications.map((app) => {
                      ...
                      return (
                        <tr key={app._id} className="hover:bg-zinc-50 transition-colors">
                          <td className="py-4 px-4 font-mono font-bold text-zinc-650">{app.applicationId}</td>
                ...
                <table className="w-full text-left text-xs text-zinc-700">
                  <thead className="text-[10px] font-bold text-zinc-500 uppercase border-b border-zinc-100">
                    <tr>
                      ...
                      <th className="py-3.5 px-4">Size / Layout</th>
                      <th className="py-3.5 px-4">Zoning State</th>
                      <th className="py-3.5 px-4 text-right">Portfolio Controls</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-100 font-medium">
                    {properties.map((prop) => (
                      <tr key={prop._id} className="hover:bg-zinc-50 transition-colors">
                        <td className="py-4 px-4">
            ...
            {inquiries.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs text-zinc-700">
                  ...
                  <tbody className="divide-y divide-zinc-100 font-medium">
                    {inquiries.map((inq) => (
                      <tr key={inq._id} className="hover:bg-zinc-50 transition-colors">
